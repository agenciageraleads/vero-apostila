...
p.title = "Reforma Tributária para Corretores de Imóveis — v5";
...
conduz.forEach((c,i)=>{ let x=MX+i*(kw+0.5);
  ...
  s.addText(c[2],{x:x+0.35,y:ky+1.55,w:kw-0.65,h:2.25,fontFace:BODY_F,fontSize:11.5,color:c[3]?MUTE:BODY,italic:c[3],paraSpaceAfter:5,margin:0});
});
footer(s);
...
const tl=[["2026","Início da transição","O novo imposto começa a ser cobrado em teste"],
...
// ============ S3 — Vários impostos viram um só (enxuto, sem decorar sigla) ============
s=p.addSlide(); bg(s);
title(s,"Vários impostos viram um só","O que entra no lugar");
card(s,MX,2.6,5.0,3.0,PANEL);
s.addText("HOJE",{x:MX+0.4,y:2.85,w:4.2,h:0.35,fontFace:BODY_F,fontSize:11,bold:true,color:MUTE,charSpacing:2,margin:0});
s.addText([
  {text:"PIS",options:{breakLine:true,bullet:true}},
  {text:"COFINS",options:{breakLine:true,bullet:true}},
  {text:"ICMS",options:{breakLine:true,bullet:true}},
  {text:"ISS  — o que você paga hoje",options:{bullet:true}},
],{x:MX+0.4,y:3.3,w:4.3,h:2.1,fontFace:BODY_F,fontSize:15,color:BODY,paraSpaceAfter:7,margin:0});
s.addText("→",{x:MX+5.2,y:3.7,w:1.2,h:0.8,fontFace:TITLE_F,fontSize:40,bold:true,color:GOLD_DK,align:"center",margin:0});
s.addShape(p.shapes.RECTANGLE,{x:MX+6.5,y:2.6,w:W-MX-(MX+6.5),h:3.0,fill:{color:NAVY},shadow:mkShadow()});
s.addText("O NOVO IMPOSTO",{x:MX+6.9,y:2.95,w:4.5,h:0.35,fontFace:BODY_F,fontSize:11,bold:true,color:GOLD,charSpacing:2,margin:0});
s.addText("Um só",{x:MX+6.9,y:3.4,w:4.5,h:0.9,fontFace:TITLE_F,fontSize:40,bold:true,color:CREAM,margin:0});
s.addText("Chamado IBS + CBS. Você não precisa decorar a sigla — precisa saber que vira um imposto só, com uma lógica nova.",{x:MX+6.9,y:4.5,w:W-MX-(MX+6.9)-0.3,h:1.0,fontFace:BODY_F,fontSize:13,color:CREAM,margin:0});
s.addText("A alíquota de referência estimada é ~26,5%. Começa em 2026 e completa em 2033.",
...
// ============ S5 — E você? (impacto por setor reformulado) ============
s=p.addSlide(); bg(s);
title(s,"Não atinge todo mundo igual","E você, onde está?");
card(s,MX,2.4,3.4,1.5,PANEL);
s.addText("QUEM SENTE MENOS",{x:MX+0.3,y:2.6,w:2.8,h:0.3,fontFace:BODY_F,fontSize:9,bold:true,color:GREEN,charSpacing:1.5,margin:0});
s.addText("Indústria",{x:MX+0.3,y:2.95,w:2.8,h:0.5,fontFace:TITLE_F,fontSize:18,bold:true,color:NAVY,margin:0});
s.addText("Muito insumo pra descontar.",{x:MX+0.3,y:3.45,w:2.9,h:0.4,fontFace:BODY_F,fontSize:10.5,color:BODY,margin:0});
card(s,MX,4.05,3.4,1.5,REDBG);
s.addText("QUEM SENTE MAIS",{x:MX+0.3,y:4.25,w:2.8,h:0.3,fontFace:BODY_F,fontSize:9,bold:true,color:RED,charSpacing:1.5,margin:0});
s.addText("Serviços",{x:MX+0.3,y:4.6,w:2.8,h:0.5,fontFace:TITLE_F,fontSize:18,bold:true,color:NAVY,margin:0});
s.addText("Quase nada pra descontar. A alíquota sobe.",{x:MX+0.3,y:5.1,w:2.9,h:0.5,fontFace:BODY_F,fontSize:10.5,color:BODY,margin:0});
let bvx=MX+4.0;
s.addShape(p.shapes.RECTANGLE,{x:bvx,y:2.4,w:W-MX-bvx,h:3.2,fill:{color:NAVY},shadow:mkShadow()});
s.addShape(p.shapes.RECTANGLE,{x:bvx,y:2.4,w:W-MX-bvx,h:0.4,fill:{color:GOLD}});
s.addText("E VOCÊ?",{x:bvx+0.5,y:2.95,w:W-MX-bvx-1,h:0.45,fontFace:BODY_F,fontSize:13,bold:true,color:GOLD,charSpacing:3,margin:0});
s.addText("Corretor",{x:bvx+0.5,y:3.45,w:W-MX-bvx-1,h:1.1,fontFace:TITLE_F,fontSize:54,bold:true,color:CREAM,margin:0});
s.addText("Corretagem é serviço. Você está no lado que mais sente — e é por isso que esta sessão é sobre você.",{x:bvx+0.5,y:4.8,w:W-MX-bvx-1,h:0.7,fontFace:BODY_F,fontSize:14,color:CREAM,margin:0});
footer(s);
...
s.addText("Quem vende produto desconta o que compra. O Carlos vende conhecimento — e quase não tem o que descontar. Por isso o serviço paga sobre quase tudo que entra (alíquota de referência ~26,5%).",
  {x:MX,y:6.05,w:W-2*MX,h:0.6,fontFace:BODY_F,fontSize:13,italic:true,color:BODY,align:"center",margin:0});
footer(s);

// ============ S7 — A conta do Carlos: hoje x depois (+ a decisão) ============
s=p.addSlide(); bg(s);
...
s.addText("Quando o contador mostrou essa conta, o Carlos teve duas opções: esperar a regra pegar — ou agir agora. Guarde isso.",
  {x:MX,y:5.6,w:W-2*MX,h:0.6,fontFace:BODY_F,fontSize:13,italic:true,bold:true,color:NAVY,margin:0});
footer(s);
...
// ============ S7c — A carteira de locação (Carlos administra 18 aluguéis) ============
s=p.addSlide(); bg(s);
title(s,"A carteira de locação","O aluguel no dia a dia");
s.addText("VALIDAR",{x:W-2.6,y:0.6,w:1.7,h:0.3,fontFace:BODY_F,fontSize:9,bold:true,color:GOLD_DK,charSpacing:2,align:"right",margin:0});
s.addText("Lembra do Carlos? Além de vender, ele administra 18 aluguéis. Um deles: R$ 2.500/mês, taxa de 10% (R$ 250). Confirme os números com seu contador.",{x:MX,y:1.85,w:W-2*MX,h:0.35,fontFace:BODY_F,fontSize:12.5,color:GOLD_DK,bold:true,margin:0});
...
s.addText([
  ...
  {text:"A mordida na taxa de administração sobe.",options:{bullet:true}},
...
s.addText("Na carteira do Carlos, a conta tem dois donos: o serviço da imobiliária e o aluguel. Quem não souber quem absorve a alta, descobre no reajuste.",
  {x:MX,y:6.05,w:W-2*MX,h:0.6,fontFace:BODY_F,fontSize:13.5,italic:true,color:BODY,align:"center",margin:0});
footer(s);

// ============ S8 — Corretor de imóveis (CIB) ============
s=p.addSlide(); bg(s);
title(s,"O que muda para o corretor de imóveis","Onde a fiscalização aperta");
s.addText("O apartamento que o Carlos vendeu agora tem um “CPF” próprio — e conversa direto com o fisco.",{x:MX,y:1.9,w:W-2*MX,h:0.35,fontFace:BODY_F,fontSize:12.5,color:GOLD_DK,bold:true,margin:0});
card(s,MX,2.55,5.6,3.55); goldEdge(s,MX,2.55,3.55);
...
],{x:MX+0.35,y:3.5,w:5.0,h:2.4,fontFace:BODY_F,fontSize:13.5,color:NAVY,paraSpaceAfter:8,margin:0});
let rx=MX+6.1;
card(s,rx,2.55,W-MX-rx,3.55);
...
],{x:rx+0.35,y:3.5,w:W-MX-rx-0.7,h:2.4,fontFace:BODY_F,fontSize:13.5,color:BODY,paraSpaceAfter:8,margin:0});
footer(s);

// ============ S9 — MEI (Carlos também achava que podia) ============
s=p.addSlide(); bg(s);
...
s.addText([
  {text:"O Carlos também achava que podia ser MEI. Muita gente opera assim — e descobre tarde.",options:{breakLine:true,bullet:true,color:NAVY}},
...
// ============ S11 — Consequências + a decisão do Carlos ============
s=p.addSlide(); bg(s);
...
s.addText("O Carlos teve duas opções: ser esse corretor — ou o do próximo slide.",{x:MX,y:6.05,w:W-2*MX,h:0.5,fontFace:TITLE_F,fontSize:18,italic:true,bold:true,color:RED,align:"center",margin:0});
footer(s);

// ============ S13b — O corretor preparado faz 4 coisas (Carlos escolheu este lado) ============
s=p.addSlide(); bg(s);
title(s,"O corretor preparado faz 4 coisas","O Carlos escolheu este lado");
const fazem=[
...
// ============ S13 — 3 perguntas pro seu contador (o telefonema do Carlos) ============
s=p.addSlide(); bg(s);
title(s,"3 perguntas pro seu contador","O primeiro telefonema do Carlos");
...
s=p.addSlide(); bg(s);
s.addText("RAIO-X DO CORRETOR PÓS-REFORMA",{x:MX,y:2.05,w:11,h:0.4,fontFace:BODY_F,fontSize:12,bold:true,color:GOLD_DK,charSpacing:3,margin:0});
goldEdge(s,MX,2.6,1.0);
s.addText("Onde você é o Carlos?",{x:MX+0.3,y:2.55,w:11,h:1.0,fontFace:TITLE_F,fontSize:44,bold:true,color:NAVY,margin:0});
s.addText("Durante uma hora vocês acompanharam o Carlos. Agora vocês têm cinco minutos para descobrir onde vocês são o Carlos. Pegue a folha — é o seu raio-x.",
  {x:MX+0.3,y:3.85,w:10.6,h:1.3,fontFace:BODY_F,fontSize:18,color:BODY,margin:0});
footer(s);
...
s=p.addSlide(); bg(s);
s.addText("FECHAMENTO",{x:MX,y:1.4,w:6,h:0.4,fontFace:BODY_F,fontSize:12,bold:true,color:GOLD_DK,charSpacing:4,margin:0});
goldEdge(s,MX,1.95,1.4);
s.addText("Daqui a alguns anos, ninguém vai te perguntar qual era a alíquota do IBS.",
  {x:MX+0.3,y:1.9,w:11.2,h:0.9,fontFace:TITLE_F,fontSize:24,bold:true,color:NAVY,margin:0});
s.addText("Mas muita gente vai lembrar do ano em que começou a sobrar menos. Uns vão dizer: “a reforma acabou com a minha margem.” Outros: “foi o ano em que aprendi a administrar o meu negócio.”",
  {x:MX+0.3,y:3.1,w:11,h:1.4,fontFace:BODY_F,fontSize:17,color:BODY,margin:0});
s.addText("A lei é a mesma. O imposto é o mesmo. O que muda é o empresário.",
  {x:MX+0.3,y:4.95,w:11,h:0.9,fontFace:TITLE_F,fontSize:24,italic:true,bold:true,color:GOLD_DK,margin:0});
footer(s);

// ============ Espelho — quem era o Carlos ============
s=p.addSlide(); bg(s);
s.addText("UMA ÚLTIMA COISA",{x:MX,y:1.1,w:8,h:0.4,fontFace:BODY_F,fontSize:12,bold:true,color:GOLD_DK,charSpacing:3,margin:0});
goldEdge(s,MX,1.65,1.4);
s.addText("Em algum momento dessa história, você se viu no Carlos?",
  {x:MX+0.3,y:1.6,w:11.2,h:1.0,fontFace:TITLE_F,fontSize:27,bold:true,color:NAVY,margin:0});
s.addText("O Carlos nunca existiu. Ele é feito de pedaços de muitos corretores que a gente conhece. Talvez, hoje, ele tenha tido o seu nome.",
  {x:MX+0.3,y:2.95,w:11,h:1.3,fontFace:BODY_F,fontSize:18,color:BODY,margin:0});
card(s,MX,4.5,W-2*MX,1.25,NAVY);
s.addText("A reforma não quebra quem é organizado. Ela expõe quem vivia no improviso.",
  {x:MX+0.4,y:4.5,w:W-2*MX-0.8,h:1.25,fontFace:TITLE_F,fontSize:19,bold:true,italic:true,color:CREAM,align:"center",valign:"middle",margin:0});
s.addText("A escolha do Carlos — e a sua — começa agora.",{x:MX,y:6.05,w:W-2*MX,h:0.4,fontFace:BODY_F,fontSize:14,italic:true,color:GOLD_DK,align:"center",margin:0});
footer(s);
...
const out="/Users/Lucas-Lenovo/Vero - Escola de Empresários/_imersoes/reforma-tributaria-corretores/deck/Vero_Reforma_Tributaria_Corretores_v5.pptx";
